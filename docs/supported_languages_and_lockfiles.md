...
| Dart       | `pubspec.lock`<br>`pubspec.lock`                                                                                                                                         |
| Elixir     | `mix.lock`                                                                                                                                             |
| Go         | `go.mod`                                                                                                                                               |
...
To scan the dependency versions selected for an environment, prefer a resolved
lockfile. If a supported lockfile is not available, use a fully pinned manifest
generated from the environment, such as a `requirements.txt` file generated by
`pip freeze`.

Manifest files can declare version ranges and do not necessarily record the
versions installed in an environment. OSV-Scanner extracts versions from
manifests on a best-effort basis. The exact behavior varies by ecosystem and
extractor, so a version reported from a manifest may not be the version that is
installed. See OSV-Scalibr's
[language extractors](https://github.com/google/osv-scalibr/tree/main/extractor/filesystem/language)
and their tests for the current extraction logic.

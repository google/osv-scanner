...
OSV-Scanner now supports transitive dependency scanning for Maven pom.xml. This feature is enabled by default when scanning, but it is disabled in the [experimental offline mode](./offline-mode.md).

OSV-Scanner uses [deps.dev’s resolver library](https://github.com/google/deps.dev/tree/main/util/resolve) to compute the dependency graph of a project. This graph includes all of the direct and transitive dependencies. Requirements are provided by [deps.dev API](https://docs.deps.dev/api/v3/#getrequirements).

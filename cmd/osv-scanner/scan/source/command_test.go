...
func TestCommand_UpdateConfigIgnores_All(t *testing.T) {
	...
	tests := []withFilesToRemove{
		...
		},
		{
			Name: "deep",
			Args: []string{
				"", "source", "--format=vertical", "--experimental-update-config-ignores=all", "-r",
			},
		},
		{
		...
		},
		{
			...
			Args: []string{
				"", "source", "--format=vertical", "--experimental-update-config-ignores=all", "--config", "./testdata/locks-with-many-configs/custom-config.toml",
			},
		},
		{
			Name: "global_config_deep",
			Args: []string{
				"", "source", "--format=vertical", "--experimental-update-config-ignores=all", "-r", "--config", "./testdata/locks-with-many-configs/custom-config.toml",
			},
		},
	}
	for _, tt := range tests {
		t.Run(tt.Name, func(t *testing.T) {
			...
			err := os.CopyFS(testDir, os.DirFS("./testdata/locks-with-many-configs"))
			if err != nil {
				t.Fatal(err)
			}

	...
	tests := []cliTestCase{
		...
		// when an explicit parse-as is given, it's applied to that file
		{
			...
			wantStdout: `
				...
				| OSV URL (ID IN BOLD)                | ECOSYSTEM | PACKAGE   | VERSION | FIXED VERSIONS | SOURCE                                       |
				+-------------------------------------+-----------+-----------+---------+----------------+----------------------------------------------+
				| https://osv.dev/GHSA-whgm-jr23-g3j9 | npm       | ansi-html | 0.0.1   | 0.0.8          | fixtures/locks-insecure/my-package-lock.json |
				+-------------------------------------+-----------+-----------+---------+----------------+----------------------------------------------+
			`,
			wantStderr: "",
		},
		// multiple, + output order is deterministic
		{
			...
			wantStdout: `
				...
				+-------------------------------------+-----------+-----------+---------+----------------+----------------------------------------------+
				| https://osv.dev/GHSA-whgm-jr23-g3j9 | npm       | ansi-html | 0.0.1   | 0.0.8          | fixtures/locks-insecure/my-package-lock.json |
				| https://osv.dev/GHSA-whgm-jr23-g3j9 | npm       | ansi-html | 0.0.1   | 0.0.8          | fixtures/locks-insecure/my-yarn.lock         |
				+-------------------------------------+-----------+-----------+---------+----------------+----------------------------------------------+
				`,
			wantStderr: "",
		},

...
func parseLocalVersion(local string) (parts []string) {
	for _, part := range utility.CachedRegexMustCompile(`[._-]`).Split(local, -1) {
...
func normalizePyPILegacyPart(part string) string {
	...
	if utility.CachedRegexMustCompile(`\d`).MatchString(part[:1]) {
...
func parsePyPIVersionParts(str string) (parts []string) {
	re := utility.CachedRegexMustCompile(`(\d+|[a-z]+|\.|-)`)
...
func parsePyPIVersion(str string) PyPIVersion {
	...
	re := utility.CachedRegexMustCompile(`^\s*v?(?:(?:(?P<epoch>[0-9]+)!)?(?P<release>[0-9]+(?:\.[0-9]+)*)(?P<pre>[-_\.]?(?P<pre_l>(a|b|c|rc|alpha|beta|pre|preview))[-_\.]?(?P<pre_n>[0-9]+)?)?(?P<post>(?:-(?P<post_n1>[0-9]+))|(?:[-_\.]?(?P<post_l>post|rev|r)[-_\.]?(?P<post_n2>[0-9]+)?))?(?P<dev>[-_\.]?(?P<dev_l>dev)[-_\.]?(?P<dev_n>[0-9]+)?)?)(?:\+(?P<local>[a-z0-9]+(?:[-_\.][a-z0-9]+)*))?\s*$`)

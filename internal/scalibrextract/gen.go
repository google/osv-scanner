...
func main() {
	...
	for _, name := range names {
		output += "\n"
		output += "var Extractors" + name + " = []string{"

		slices.Sort(presets[name])

		for _, extractor := range presets[name] {
			currentNamespace := strings.Split(extractor, "/")[0]

			if previousNamespace != currentNamespace {
				output += "\n"
			}
	...
	err = ioutil.WriteFile("presets.go", formatted, 0644)
	if err != nil {
		panic(err)
	}
}

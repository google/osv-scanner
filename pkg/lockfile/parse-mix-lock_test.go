...
func TestParseMixLockFile_FileDoesNotExist(t *testing.T) {
	...
	packages, diag, err := lockfile.ParseMixLockFile("fixtures/mix/does-not-exist")
...
func TestParseMixLockWithDiagnostics(t *testing.T) {
	t.Parallel()

	testParser(t,
		lockfile.ParseMixLockFile,
		lockfile.ParseMixLockWithDiagnostics,
		[]testParserWithDiagnosticsTest{
			// one package
			{
				name: "",
				file: "fixtures/mix/one-package.lock",
			...
			{
				name: "",
				file: "fixtures/mix/two-packages.lock",
				want: []lockfile.PackageDetails{
					{
						Name:      "plug",
						Version:   "1.11.1",
